...
# -*- coding: utf-8 -*-
# 01_Qwen3.py
# 在本地 CPU 笔记本上运行 Qwen3-0.6B 模型的脚本
...
class FeedForward(nn.Module):
    # 这个模块实现了一个带有 SiLU 激活和 SwiGLU 结构的 前馈网络（FFN），
    # 这是现代 Transformer 模型（如 Llama、Mistral）中常用的 FFN 变体。
...
class RMSNorm(nn.Module):
    # RMSNorm (Root Mean Square Normalization) 是一种用于 Transformer 模型的高效归一化方法，
    # 相比 LayerNorm 更快且性能相似。
...
# GQA 是一种高效的注意力机制，通过重用 Key (K) 和 Value (V) 投影，显著减少了内存带宽需求，
# 同时保持接近多头注意力 (MHA) 的性能，是 MHA 和 多查询注意力 (MQA) 之间的一种折衷方案。
...
if CHOOSE_MODEL == "0.6B":
    QWEN3_CONFIG = {
        ...
        "dtype": torch.float32,   # 可在 CPU 上改为 torch.float32
    }
...
# 快速检查前向传播是否正常
test_input = torch.tensor([1, 2, 3]).unsqueeze(0)
test_output = model(test_input)
print("Test forward output shape:", test_output.shape)
